	...
	class PDLParser
	{
		...
		Parameters ConsumeBlock(bool consumeSemicolon)
		{
			...
			while (!stream.EndOfStream)
			{
				...
				if (c == '\n' || c == '\r')
				{
					if (c == '\r') if (!stream.EndOfStream && stream.Peek() == '\n') stream.Read();
					line++;
					continue;
				}
		...
		void ConsumeNumber(char c0, Parameters ret, string name)
		{
			...
			if (number.StartsWith("0x"))
			{
				...
				long val = 0;
				if (!long.TryParse(number.Substring(2), NumberStyles.AllowHexSpecifier, NeutralCulture, out val)) throw new TextParserException("Invalid character while parsing the value of a parameter.", line);
				ret.Add(name, val);
			}
			else
			{
				...
				double dval = 0;
				if (!double.TryParse(number, NumberStyles.Float, NeutralCulture, out dval)) throw new TextParserException("Invalid character while parsing the value of a parameter.", line);
				ret.Add(name, dval);
			}
		}
		...
		void ConsumeNumberList(char c0, Parameters ret, string name)
		{
			...
			if (!(c0 == '+' || c0 == '-' || (c0 >= '0' && c0 <= '9'))) throw new ArgumentException("Must be either '+', '-', or between '0' and '9'.", "c0");
